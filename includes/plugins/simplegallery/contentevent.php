<?php
	...
	$galleries_ = $config['galleries']['gallerie'];

    $pagegallery = '';

    if(isset($galleries_[0])) {  // is array because there is more than one gallery
        foreach($galleries_ as $gallerie) {
            if(isset($gallerie['page']) && $gallerie['page'] == $page->id) {
                $pagegallery = $gallerie['id'];
                $gallery_ = $gallerie;
            }
        }
    }
    else {    // is not an array because there is only one gallery
        if(isset($galleries_['page']) && $galleries_['page'] == $page->id) {
            $pagegallery = $galleries_['id'];
            $gallery_ = $galleries_;
        }
    }
    ...
    if($pagegallery) {
        ...
        $data_g = implode("", file($gallery_['xml']));
